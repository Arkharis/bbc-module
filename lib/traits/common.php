...
trait Common
{
			...
			foreach ($this->checkParams as $key => $params) {
				switch ($params['type']) {
					...
					case 'string':
						...
						if ($this->arParams[$key] == '' && $params['error'] !== false) {
	...
	private function startAjax()
	{
		...
		if ($this->arParams['AJAX_PARAM_NAME'] == '') {
			$this->arParams['AJAX_PARAM_NAME'] = 'compid';
		}

		if ($this->arParams['AJAX_COMPONENT_ID'] == '') {
	...
	protected function catchException(\Exception $exception, $notifier = null)
	{
		...
		if (($notifier === true) || ($this->exceptionNotifier && $notifier !== false) && (defined('BX_EXC_NOTIFY') && BX_EXC_NOTIFY !== false)) {
